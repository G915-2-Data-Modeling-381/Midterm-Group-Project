...
CREATE Domain sdMiscellanousStrings_AG.classTime_AN
	AS CHAR(5) NULL
	CHECK (VALUE LIKE '[1-9]:[0-5][0-9]' OR VALUE LIKE '[1-2][0-9]:[0-5][0-9]');
...
CREATE Domain sdAddressString_AN.postalCode_AN
	AS CHAR(5) NULL
	CHECK (VALUE LIKE '[0-9][0-9][0-9][0-9][0-9]');
...
CREATE Domain dDatetime.sdNanosecondPreciseDatetime_JG
	AS TIMESTAMP NULL
	CHECK (pg_typeof(to_date(VALUE, 'YYYY-MM-DD HH12:MI:US') = date);
...
CREATE FUNCTION Hashing.CreateBYTEARowSha2_256HashKey() AS $$CREATE FUNCTION CreateBYTEARowSha2_256HashKey ()  
...
CREATE FUNCTION Hashing.CreateBYTEASha2_256HashKey() AS $$CREATE FUNCTION CreateBYTEASha2_256HashKey ()  
...
( 
	...
	Salary               integer  NULL ,
	BirthDate            date  NULL   DEFAULT  '19000101',
	HireDate             date  NULL   DEFAULT  CONVERT (date, statement_timestamp()),
	PhoneNumber          char(14)  NULL ,
...
( 
	...
	EmployeeHireDate     date  NULL   DEFAULT  CONVERT (date, statement_timestamp()),
